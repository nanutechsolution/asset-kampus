<div class="space-y-6 animate-fade-in">
    ...
    <div class="bg-white/80 backdrop-blur-xl shadow-sm border border-gray-100/50 rounded-3xl p-3 flex flex-col sm:flex-row justify-between items-center gap-4 transition-all z-20 relative">
        <div class="relative w-full sm:w-80 group">
            ...
            <input wire:model.live.debounce.300ms="search" type="text" class="block w-full pl-12 pr-4 py-3 border-0 bg-gray-50/50 hover:bg-gray-100/50 focus:bg-white text-gray-900 rounded-2xl ring-1 ring-inset ring-gray-200 focus:ring-2 focus:ring-inset focus:ring-unmaris-500 sm:text-sm transition-all shadow-sm" placeholder="Cari nama atau email...">
        </div>

        <a href="{{ route('users.form') }}" wire:navigate class="w-full sm:w-auto inline-flex justify-center items-center px-6 py-3 border border-transparent shadow-[0_4px_12px_rgba(2,132,199,0.2)] text-sm font-bold rounded-2xl text-white bg-gradient-to-r from-unmaris-600 to-unmaris-500 hover:from-unmaris-700 hover:to-unmaris-600 transition-all hover:scale-[1.02] active:scale-[0.98] italic">
    ...
    <div class="hidden md:block bg-white shadow-sm ring-1 ring-gray-100 rounded-[2rem] overflow-hidden relative z-10">
        <table class="min-w-full divide-y divide-gray-100 text-sm">
            <thead class="bg-gray-50/80 backdrop-blur-sm">
                <tr>
                    <th class="py-4 pl-6 pr-3 text-left text-xs font-bold text-gray-500 uppercase tracking-widest italic">Info Pengguna</th>
                    <th class="px-3 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-widest italic">Peran</th>
                    <th class="px-3 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-widest italic">Status Akses</th>
                    <th class="py-4 pl-3 pr-6 text-right font-bold text-gray-500 uppercase italic">Aksi</th>
                </tr>
            </thead>
            <tbody class="divide-y divide-gray-50 bg-white italic">
                ...
                <tr class="hover:bg-unmaris-50/30 transition-colors group">
                    ...
                    <td class="py-4 pl-3 pr-6 whitespace-nowrap text-right">
                        <div class="opacity-0 group-hover:opacity-100 transition-opacity">
                            <a href="{{ route('users.form', $user->id) }}" wire:navigate class="p-2 text-unmaris-600 hover:bg-unmaris-50 rounded-xl inline-block transition-colors">
        ...
        <div class="bg-white rounded-3xl p-5 shadow-sm border border-gray-100/50 relative overflow-hidden">
            ...
            <div class="flex justify-between items-start mb-4">
                <div class="flex items-center gap-3">
                    ...
                    <div class="min-w-0">
                        <h3 class="text-sm font-bold text-gray-900 leading-tight truncate italic">{{ $user->name }}</h3>
            ...
            <div class="flex items-center justify-between pt-4 border-t border-gray-50">
                <div class="flex items-center gap-2">
                    <span class="text-[10px] font-bold text-gray-400 uppercase italic">Akses:</span>
                ...
                <a href="{{ route('users.form', $user->id) }}" wire:navigate class="text-[11px] font-bold text-unmaris-600 bg-unmaris-50 px-3 py-1.5 rounded-xl italic">
                    Edit & Password
                </a>
            </div>
        </div>
